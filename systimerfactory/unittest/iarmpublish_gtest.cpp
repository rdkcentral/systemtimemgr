...
extern "C" {
    ...
    IARM_Result_t IARM_Bus_IsConnected(const char* name, int* isRegistered) {
      ...
      IARM_Result_t IARM_Bus_BroadcastEvent(const char* owner, IARM_EventId_t event, void* data, size_t len) {
...
TEST_F(IarmPublishTest, Publish_CallsBroadcastEventWithCorrectArgs) {
    int reg = 1;
    EXPECT_CALL(mock, IARM_Bus_IsConnected(::testing::StrEq("TestPublisher"), _))
        .WillOnce(::testing::DoAll(::testing::SetArgPointee<1>(reg), ::testing::Return(IARM_RESULT_SUCCESS)));

    EXPECT_CALL(mock, IARM_Bus_BroadcastEvent(::testing::StrEq("TestPublisher"), 100, _, sizeof(TimerMsg)))
        .Times(1);
    ...
    msg.event = 100;
    msg.quality = 50;
    strncpy(msg.message, "Hello", sizeof(msg.message));
